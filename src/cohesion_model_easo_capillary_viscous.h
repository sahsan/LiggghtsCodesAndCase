...
Later edit is done by Syed N. Ahsan to incorporate heat transfer in the existing bridge model
email: 
syed.ahsan@nrel.gov
sahsan@nevada.unr.edu
snahsan.me05@gmail.com
  ...
  public:
    ...
    void connectToProperties(PropertyRegistry & registry)
    {
      ...
      if(!fix_ste)
      {
        ...
        const char * newarg[16];      //SN_Ahsan edit
    ...
    void surfacesClose(SurfacesCloseData & scdata, ForceData & i_forces, ForceData & j_forces)
    {
      ...
      double *Temp = fix_temperature->vector_atom;    //SN_Ahsan edit
      ...
      bool bridge_active = false, bridge_breaks = false, bridge_breaks_temp = false;
      ...
      if (dist > (maxSeparationDistanceRatio-1.0)*(radi+radj) && MathExtraLiggghts::compDouble(contflag[0],1.0,1e-6)) // in this case always break
      {
        ...
        bridge_breaks = true;
      else if (Temp[i]>=280.00 || Temp[j]>=280.00)
	bridge_breaks_temp = true;
      ...
      else if(bridge_breaks_temp)     //SN_Ahsan edit
	{
	  if(scdata.contact_flags) *scdata.contact_flags &= ~CONTACT_COHESION_MODEL;
